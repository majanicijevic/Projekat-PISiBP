...
export default function DashProfile() {
    ...
    uploadTask.on(
      ...
      (error) => {
        setImageFileUploadError(
          'Nije moguce promeniti sliku (Fajl mora biti manji od 2MB)'
        );
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(formData).length === 0) {
      setUpdateUserError('Nisu napravljene promene');
      return;
    }
    if (imageFileUploading) {
      setUpdateUserError('Molimo sacekajte da se ucita slika');
      return;
    }
    try {
      ...
      if (!res.ok) {
        ...
        setUpdateUserSuccess("Profil je uspesno azuriran");
      }
    } catch (error) {
        ...
        <Modal.Body>
          <div className='text-center'>
            ...
            <h3 className='mb-5 text-lg text-gray-600 dark:text-gray-400'>
              Da li ste sigurni da zelite da obrisete ovaj profil?
            </h3>
